...
export default function App() {
  ...
  if (isAdmin)
    menuItems.splice(1, 0, { id: 'admin', label: 'Staff', icon: '👥' });
    ...
    <div className="flex h-screen bg-[#0b1121] text-slate-200 font-sans overflow-hidden">
      ...
      <aside className="hidden md:flex w-72 bg-[#151c2c] border-r border-slate-800 flex-col z-20 shadow-2xl">
        <div className="p-8 border-b border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-blue-400 mt-0.5 uppercase tracking-widest font-bold">
                Sub-19 • AF Porto
              </p>
            </div>
          </div>
        ...
        <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
          <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4 px-4">
            Menu Principal
          </div>
          {menuItems.map((item) => (
            <button
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto p-4 md:p-8 text-slate-900 pb-24 md:pb-12 custom-scrollbar">
          <div className="max-w-7xl mx-auto">
            ...
            {activeTab === 'admin' && isAdmin && (
              <AdminModule
                staff={staffList}
                onAddStaff={(s) => setStaffList([...staffList, s])}
              />
            )}
            {activeTab === 'players' && (
              <PlayersModule
                players={playersList}
                onAddPlayer={(p) => setPlayersList([...playersList, p])}
              />
            )}
            {activeTab === 'training' && (
              <TrainingModule players={playersList} />
            )}
            {activeTab === 'match' && (
              <MatchModule
                players={playersList}
                reports={matchReports}
                onAddReport={(r) => setMatchReports([r, ...matchReports])}
              />
            )}
            {activeTab === 'tactics' && <TacticalBoard />}
            {activeTab === 'stats' && (
              <StatsModule players={playersList} reports={matchReports} />
            )}
          </div>
        </main>
          ...
          {menuItems.map((item) => (
            ...
            >
              ...
              <span className="text-[9px] font-bold tracking-wider uppercase truncate max-w-full">
                {item.label.split(' ')[0]}{' '}
                {/* Mostra só a primeira palavra para caber */}
              </span>
            </button>
          ))}
        </nav>
